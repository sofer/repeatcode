...
var not_finished = function (question) {
  if (question.status === 'end') {
    $('#response').hide();
    $('#completed').show();
    return false;
  } else {
...
var get_next_question = function (excluded_id) {
  ...
  $.ajax({
		url: json_url,
		data: "exclude=" + excluded_id,
		success: function(json){
	  	if (not_finished(json)) {
				loading(false);
				next = json;
			}
		},
...
var xget_next_question = function (excluded_id) {
	loading(true);
...
alert(question + " :: " + question.topic + " :: " + question.question + " :: " + question.exercise );
	$(".wrong").html('');
